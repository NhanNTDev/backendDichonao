...
namespace DiCho.Core.Utilities
{
    public static class LinQUtils
    {
        public static IQueryable<TEntity> DynamicFilter<TEntity>(this IQueryable<TEntity> source, TEntity entity)
        {
            ...
            foreach (var item in properties)
            {
                ...
                {
                    var array = (IQueryable)propertyVal;
                    source = source.Where($"{item.Name}.Any(a=> @0.Contains(a)) ", array);
                    //source = source.Where($"{item.Name}.Intersect({array}).Any()",);
                }
                //else if (item.CustomAttributes.Any(a => a.AttributeType == typeof(TimeRangeAttribute)))
                //{
                //    var timeRange = (TimeRange)propertyVal;
                //    if (timeRange.StartTime == null || timeRange.EndTime == null) continue;
                //    var field = item.CustomAttributes.FirstOrDefault(a => a.AttributeType == typeof(TimeRangeAttribute))
                //        .ConstructorArguments.FirstOrDefault().Value.ToString();
                //    if (timeRange.StartTime == timeRange.EndTime) timeRange.EndTime = timeRange.StartTime.Value.AddDays(1).Date;

                //    source = source.Where($"{field} >= @0 && {field} <= @1", timeRange.StartTime.Value, timeRange.EndTime.Value);
                //    //source = source.Where($"{item.Name}.Intersect({array}).Any()",);
                //}
                ...
                {
                    source = source.Where($"{item.Name} = \"{Guid.Parse(propertyVal.ToString())}\""); ;
                }
                else
                {
        ...
        public static (int, List<TResult>) PagingList<TResult>(this List<TResult> source, int page, int size,
           int limitPaging, int defaultPaging)
        {
            if (size > limitPaging)
            {
